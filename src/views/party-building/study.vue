<template>
    <div class="scroll-container-1_1666">
        <div id="1_1666" class="Pixso-frame-1_1666">
            <div id="1_1667" class="Pixso-vector-1_1667"></div>
            <div id="1_1668" class="Pixso-vector-1_1668"></div>
            <div id="1_1671" class="Pixso-vector-1_1671"></div>
            <p id="1_1674" class="Pixso-paragraph-1_1674">
                {{ "四川飞豹救援" }}
            </p>
            <p id="1_1675" class="Pixso-paragraph-1_1675">
                {{ "Sichuan Feibao Rescue" }}
            </p>
            <div id="1_1684" class="Pixso-vector-1_1684"></div>
            <div id="17_10" class="Pixso-vector-17_10"></div>
            <div id="1_1690" class="Pixso-vector-1_1690"></div>
            <p id="1_1704" class="Pixso-paragraph-1_1704">
                {{ "当前位置：首页 > 党建专栏> 党员学“习”" }}
            </p>
            <div id="1_1705" class="Pixso-vector-1_1705"></div>
            <div id="1_1706" class="Pixso-vector-1_1706"></div>
            <div id="1_1707" class="Pixso-vector-1_1707"></div>
            <div id="1_1708" class="Pixso-vector-1_1708"></div>
            <div id="1_1709" class="Pixso-vector-1_1709"></div>
            <p id="1_1710" class="Pixso-paragraph-1_1710">{{ "2025-12" }}</p>
            <p id="1_1711" class="Pixso-paragraph-1_1711">{{ "06" }}</p>
            <div id="1_1712" class="Pixso-vector-1_1712"></div>
            <p id="1_1713" class="Pixso-paragraph-1_1713">{{ "2025-12" }}</p>
            <p id="1_1714" class="Pixso-paragraph-1_1714">{{ "06" }}</p>
            <div id="1_1715" class="Pixso-vector-1_1715"></div>
            <p id="1_1716" class="Pixso-paragraph-1_1716">{{ "2025-12" }}</p>
            <p id="1_1717" class="Pixso-paragraph-1_1717">{{ "06" }}</p>
            <div id="1_1718" class="Pixso-vector-1_1718"></div>
            <p id="1_1719" class="Pixso-paragraph-1_1719">{{ "2025-12" }}</p>
            <p id="1_1720" class="Pixso-paragraph-1_1720">{{ "06" }}</p>
            <p id="1_1721" class="Pixso-paragraph-1_1721">
                {{
                    "认真践行习近平总书记关于党的自我革命的重要思想李炎溪，党的十八大以来，习近平总书记站在事关党的长期..."
                }}
            </p>
            <p id="1_1722" class="Pixso-paragraph-1_1722">
                {{
                    "协同推进科学立法、严格执法、公正司法、全民守法沈春耀，法治是治国理政的基本方式。党的二十届四中全会..."
                }}
            </p>
            <p id="1_1723" class="Pixso-paragraph-1_1723">
                {{
                    "“就业是家事，更是国事。”11月1日出版的第21期《求是》杂志刊发习近平总书记重要文章《促进高质量..."
                }}
            </p>
            <p id="1_1724" class="Pixso-paragraph-1_1724">
                {{
                    "“以学铸魂，就是要做好学习贯彻新时代中国特色社会主义思想的深化、内化、转化工作，从思想上正本清源、固..."
                }}
            </p>
            <p id="1_1725" class="Pixso-paragraph-1_1725">
                {{ "认真践行习近平总书记关于党的自我革命的重要思想" }}
            </p>
            <p id="1_1726" class="Pixso-paragraph-1_1726">
                {{ "协同推进科学立法、严格执法、公正司法、全民守法" }}
            </p>
            <p id="1_1727" class="Pixso-paragraph-1_1727">
                {{ "习近平总书记深刻阐释新时代新征程就业工作的新定位、新使命" }}
            </p>
            <p id="1_1728" class="Pixso-paragraph-1_1728">
                {{ "以学铸魂，站稳人民立场" }}
            </p>
            <div id="1_1805" class="Pixso-vector-1_1805"></div>
            <div id="1_1806" class="Pixso-vector-1_1806"></div>
            <div id="1_1807" class="Pixso-vector-1_1807"></div>
            <p id="1_1808" class="Pixso-paragraph-1_1808">{{ "PBCOL" }}</p>
            <p id="1_1809" class="Pixso-paragraph-1_1809">{{ "党建专栏" }}</p>
            <p id="1_1810" class="Pixso-paragraph-1_1810">{{ "团建工作" }}</p>
            <div id="1_1811" class="Pixso-vector-1_1811"></div>
            <p id="1_1812" class="Pixso-paragraph-1_1812">{{ "党建工作" }}</p>
            <div id="1_1813" class="Pixso-vector-1_1813"></div>
            <p id="1_1814" class="Pixso-paragraph-1_1814">
                {{ "党员学“习”                  >" }}
            </p>
            <div id="1_1815" class="Pixso-vector-1_1815"></div>
            <div id="1_1816" class="Pixso-vector-1_1816"></div>
            <p id="1_1817" class="Pixso-paragraph-1_1817">{{ "党员先锋" }}</p>
            <div id="1_1818" class="Pixso-vector-1_1818"></div>
            <div id="1_1821" class="Pixso-vector-1_1821"></div>
            <div id="1_1824" class="Pixso-vector-1_1824"></div>
            <div id="1_1827" class="Pixso-vector-1_1827"></div>
            <div id="1_1830" class="Pixso-vector-1_1830"></div>
            <div id="6_18" class="Pixso-group-6_18">
                <div id="6_19" class="Pixso-vector-6_19"></div>
                <p id="6_93" class="Pixso-paragraph-6_93">{{ "共计 10 条" }}</p>
            </div>
            <div id="33_192" class="Pixso-group-33_192">
                <div id="33_193" class="Pixso-vector-33_193"></div>
                <p id="33_194" class="Pixso-paragraph-33_194">
                    {{ "请输入您要搜索的内容" }}
                </p>
                <div id="33_195" class="Pixso-vector-33_195"></div>
            </div>
            <div id="33_515" class="Pixso-vector-33_515"></div>
            <p id="33_516" class="Pixso-paragraph-33_516">{{ "概况信息" }}</p>
            <p id="33_517" class="Pixso-paragraph-33_517">{{ "队伍建设" }}</p>
            <p id="33_518" class="Pixso-paragraph-33_518">{{ "信息公开" }}</p>
            <p id="33_519" class="Pixso-paragraph-33_519">{{ "动态要闻" }}</p>
            <p id="33_520" class="Pixso-paragraph-33_520">{{ "政策法规" }}</p>
            <p id="33_521" class="Pixso-paragraph-33_521">{{ "查询系统" }}</p>
            <p id="33_522" class="Pixso-paragraph-33_522">{{ "党建专栏" }}</p>
        </div>
    </div>
</template>
<script lang="ts" setup></script>
<style>
.scroll-container-1_1666 {
    height: 100%;
    width: 100%;
    overflow: auto;
}
.Pixso-frame-1_1666 {
    width: 1920px;
    height: 1892px;
    overflow: hidden;
    position: relative;
    flex-shrink: 0;
    background-color: rgba(255, 255, 255, 1);
}
.Pixso-vector-1_1667 {
    width: 100%;
    height: 100%;
    background-image: url(@/assets/images/Vector_1_1667.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 0%;
    right: 0%;
    top: 0%;
    bottom: 0%;
}
.Pixso-vector-1_1668 {
    width: 100%;
    height: 10.25%;
    background-image: url(@/assets/images/Vector_1_1668.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 0%;
    right: 0%;
    top: 0%;
    bottom: 89.75%;
}
.Pixso-vector-1_1671 {
    width: 6.72%;
    height: 6.82%;
    background-image: url(@/assets/images/Vector_1_1671.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 6.46%;
    right: 86.82%;
    top: 1.74%;
    bottom: 91.44%;
}
.Pixso-paragraph-1_1674 {
    font-size: 53px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    line-height: 53px;
    color: rgba(217, 38, 38, 1);
    width: 16.66%;
    height: 2.8%;
    position: absolute;
    left: 14.69%;
    right: 68.65%;
    top: 3.07%;
    bottom: 94.13%;
}
.Pixso-paragraph-1_1675 {
    font-size: 28.5px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    line-height: 23.5px;
    color: rgba(217, 38, 38, 1);
    width: 16.3%;
    height: 1.27%;
    position: absolute;
    left: 14.9%;
    right: 68.8%;
    top: 6.34%;
    bottom: 92.39%;
}
.Pixso-vector-1_1684 {
    width: 100%;
    height: 3.7%;
    background-image: url(@/assets/images/Vector_1_1684.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 0%;
    right: 0%;
    top: 10.25%;
    bottom: 86.05%;
}
.Pixso-vector-17_10 {
    width: 150px;
    height: 70px;
    background-image: url(@/assets/images/Group_17_10.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 113px;
    top: 194px;
}
.Pixso-vector-1_1690 {
    width: 100%;
    height: 14.8%;
    background-image: url(@/assets/images/Vector_1_1690.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 0%;
    right: 0%;
    top: 85.2%;
    bottom: 0%;
}
.Pixso-paragraph-1_1704 {
    font-size: 20px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 20px;
    color: rgba(132, 132, 132, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 5.83%;
    right: 74.74%;
    top: 15.59%;
    bottom: 83.35%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-vector-1_1705 {
    width: 46.25%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1705.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 48.23%;
    right: 5.52%;
    top: 18.18%;
    bottom: 68.08%;
}
.Pixso-vector-1_1706 {
    width: 46.25%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1706.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 48.23%;
    right: 5.52%;
    top: 33.25%;
    bottom: 53.01%;
}
.Pixso-vector-1_1707 {
    width: 46.25%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1707.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 48.23%;
    right: 5.52%;
    top: 48.31%;
    bottom: 37.95%;
}
.Pixso-vector-1_1708 {
    width: 46.25%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1708.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 48.23%;
    right: 5.52%;
    top: 63.37%;
    bottom: 22.89%;
}
.Pixso-vector-1_1709 {
    width: 0%;
    height: 6.5%;
    background-image: url(@/assets/images/Vector_1_1709.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 54.43%;
    right: 45.57%;
    top: 21.83%;
    bottom: 71.67%;
}
.Pixso-paragraph-1_1710 {
    font-size: 20px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 20px;
    color: rgba(132, 132, 132, 1);
    width: 4.79%;
    height: 1.06%;
    position: absolute;
    left: 48.8%;
    right: 46.41%;
    top: 25.9%;
    bottom: 73.04%;
}
.Pixso-paragraph-1_1711 {
    font-size: 50px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    line-height: 50px;
    color: rgba(41, 41, 41, 1);
    width: 2.29%;
    height: 2.65%;
    position: absolute;
    left: 50.05%;
    right: 47.66%;
    top: 23.2%;
    bottom: 74.15%;
}
.Pixso-vector-1_1712 {
    width: 0%;
    height: 6.5%;
    background-image: url(@/assets/images/Vector_1_1712.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 54.43%;
    right: 45.57%;
    top: 36.89%;
    bottom: 56.61%;
}
.Pixso-paragraph-1_1713 {
    font-size: 20px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 20px;
    color: rgba(132, 132, 132, 1);
    width: 4.79%;
    height: 1.06%;
    position: absolute;
    left: 48.8%;
    right: 46.41%;
    top: 40.96%;
    bottom: 57.98%;
}
.Pixso-paragraph-1_1714 {
    font-size: 50px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    line-height: 50px;
    color: rgba(41, 41, 41, 1);
    width: 2.29%;
    height: 2.64%;
    position: absolute;
    left: 50.05%;
    right: 47.66%;
    top: 38.27%;
    bottom: 59.09%;
}
.Pixso-vector-1_1715 {
    width: 0%;
    height: 6.5%;
    background-image: url(@/assets/images/Vector_1_1715.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 54.43%;
    right: 45.57%;
    top: 51.96%;
    bottom: 41.54%;
}
.Pixso-paragraph-1_1716 {
    font-size: 20px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 20px;
    color: rgba(132, 132, 132, 1);
    width: 4.79%;
    height: 1.05%;
    position: absolute;
    left: 48.8%;
    right: 46.41%;
    top: 56.03%;
    bottom: 42.92%;
}
.Pixso-paragraph-1_1717 {
    font-size: 50px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    line-height: 50px;
    color: rgba(41, 41, 41, 1);
    width: 2.29%;
    height: 2.64%;
    position: absolute;
    left: 50.05%;
    right: 47.66%;
    top: 53.33%;
    bottom: 44.03%;
}
.Pixso-vector-1_1718 {
    width: 0%;
    height: 6.5%;
    background-image: url(@/assets/images/Vector_1_1718.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 54.43%;
    right: 45.57%;
    top: 67.02%;
    bottom: 26.48%;
}
.Pixso-paragraph-1_1719 {
    font-size: 20px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 20px;
    color: rgba(132, 132, 132, 1);
    width: 4.79%;
    height: 1.06%;
    position: absolute;
    left: 48.8%;
    right: 46.41%;
    top: 71.09%;
    bottom: 27.85%;
}
.Pixso-paragraph-1_1720 {
    font-size: 50px;
    font-family: "DINCondensedC-Regular";
    font-weight: 400;
    line-height: 50px;
    color: rgba(41, 41, 41, 1);
    width: 2.29%;
    height: 2.65%;
    position: absolute;
    left: 50.05%;
    right: 47.66%;
    top: 68.39%;
    bottom: 28.96%;
}
.Pixso-paragraph-1_1721 {
    font-size: 18px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 30px;
    color: rgba(132, 132, 132, 1);
    width: 33.64%;
    height: 3.17%;
    position: absolute;
    left: 56.15%;
    right: 10.21%;
    top: 26.16%;
    bottom: 70.67%;
}
.Pixso-paragraph-1_1722 {
    font-size: 18px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 30px;
    color: rgba(132, 132, 132, 1);
    width: 33.64%;
    height: 3.17%;
    position: absolute;
    left: 56.15%;
    right: 10.21%;
    top: 41.23%;
    bottom: 55.6%;
}
.Pixso-paragraph-1_1723 {
    font-size: 18px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 30px;
    color: rgba(132, 132, 132, 1);
    width: 33.64%;
    height: 3.17%;
    position: absolute;
    left: 56.15%;
    right: 10.21%;
    top: 56.29%;
    bottom: 40.54%;
}
.Pixso-paragraph-1_1724 {
    font-size: 18px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 30px;
    color: rgba(132, 132, 132, 1);
    width: 33.64%;
    height: 3.17%;
    position: absolute;
    left: 56.15%;
    right: 10.21%;
    top: 71.35%;
    bottom: 25.48%;
}
.Pixso-paragraph-1_1725 {
    font-size: 30px;
    font-family: "FZHei-B01S-Regular";
    font-weight: 400;
    line-height: 40px;
    color: rgba(56, 56, 56, 1);
    width: 34.38%;
    height: 4.23%;
    position: absolute;
    left: 56.09%;
    right: 9.53%;
    top: 20.77%;
    bottom: 75%;
}
.Pixso-paragraph-1_1726 {
    font-size: 30px;
    font-family: "FZHei-B01S-Regular";
    font-weight: 400;
    line-height: 40px;
    color: rgba(56, 56, 56, 1);
    width: 34.38%;
    height: 4.22%;
    position: absolute;
    left: 56.09%;
    right: 9.53%;
    top: 35.84%;
    bottom: 59.94%;
}
.Pixso-paragraph-1_1727 {
    font-size: 30px;
    font-family: "FZHei-B01S-Regular";
    font-weight: 400;
    line-height: 40px;
    color: rgba(56, 56, 56, 1);
    width: 34.38%;
    height: 4.23%;
    position: absolute;
    left: 56.09%;
    right: 9.53%;
    top: 50.9%;
    bottom: 44.87%;
}
.Pixso-paragraph-1_1728 {
    font-size: 30px;
    font-family: "FZHei-B01S-Regular";
    font-weight: 400;
    line-height: 40px;
    color: rgba(56, 56, 56, 1);
    width: 34.38%;
    height: 2.11%;
    position: absolute;
    left: 56.09%;
    right: 9.53%;
    top: 67.02%;
    bottom: 30.87%;
}
.Pixso-vector-1_1805 {
    width: 18.38%;
    height: 8.61%;
    background-image: url(@/assets/images/Vector_1_1805.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 5.94%;
    right: 75.68%;
    top: 18.13%;
    bottom: 73.26%;
}
.Pixso-vector-1_1806 {
    width: 18.38%;
    height: 6.07%;
    background-image: url(@/assets/images/Vector_1_1806.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 5.94%;
    right: 75.68%;
    top: 32.88%;
    bottom: 61.05%;
}
.Pixso-vector-1_1807 {
    width: 18.38%;
    height: 31.02%;
    background-image: url(@/assets/images/Vector_1_1807.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 5.94%;
    right: 75.68%;
    top: 51.27%;
    bottom: 17.71%;
}
.Pixso-paragraph-1_1808 {
    font-size: 75px;
    font-family: "FZHei-B01S-Regular";
    font-weight: 400;
    line-height: 75px;
    color: rgba(188, 211, 255, 1);
    width: 14.01%;
    height: 3.96%;
    position: absolute;
    left: 10.05%;
    right: 75.94%;
    top: 20.56%;
    bottom: 75.48%;
    opacity: 0.5;
}
.Pixso-paragraph-1_1809 {
    font-size: 50px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 50px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 11.2%;
    right: 78.54%;
    top: 20.24%;
    bottom: 77.11%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-1_1810 {
    font-size: 20px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 20px;
    color: rgba(255, 255, 255, 1);
    width: 4.22%;
    height: 1.06%;
    position: absolute;
    left: 8.75%;
    right: 87.03%;
    top: 35.36%;
    bottom: 63.58%;
}
.Pixso-vector-1_1811 {
    width: 18.38%;
    height: 6.08%;
    background-image: url(@/assets/images/Vector_1_1811.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 5.94%;
    right: 75.68%;
    top: 26.74%;
    bottom: 67.18%;
}
.Pixso-paragraph-1_1812 {
    font-size: 20px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 20px;
    color: rgba(255, 255, 255, 1);
    width: 4.22%;
    height: 1.06%;
    position: absolute;
    left: 8.75%;
    right: 87.03%;
    top: 29.23%;
    bottom: 69.71%;
}
.Pixso-vector-1_1813 {
    width: 18.38%;
    height: 6.08%;
    background-image: url(@/assets/images/Vector_1_1813.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 5.94%;
    right: 75.68%;
    top: 45.14%;
    bottom: 48.78%;
}
.Pixso-paragraph-1_1814 {
    font-size: 20px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 20px;
    color: rgba(255, 255, 255, 1);
    white-space: pre-wrap;
    width: 11.67%;
    height: 1.06%;
    position: absolute;
    left: 8.75%;
    right: 79.58%;
    top: 47.57%;
    bottom: 51.37%;
}
.Pixso-vector-1_1815 {
    width: 0.05%;
    height: 1.21%;
    background-image: url(@/assets/images/Vector_1_1815.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 8.07%;
    right: 91.88%;
    top: 47.52%;
    bottom: 51.27%;
}
.Pixso-vector-1_1816 {
    width: 18.38%;
    height: 6.07%;
    background-image: url(@/assets/images/Vector_1_1816.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 5.94%;
    right: 75.68%;
    top: 39.01%;
    bottom: 54.92%;
}
.Pixso-paragraph-1_1817 {
    font-size: 20px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 20px;
    color: rgba(255, 255, 255, 1);
    width: 4.22%;
    height: 1.06%;
    position: absolute;
    left: 8.75%;
    right: 87.03%;
    top: 41.49%;
    bottom: 57.45%;
}
.Pixso-vector-1_1818 {
    width: 22.03%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1818.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 26.2%;
    right: 51.77%;
    top: 18.13%;
    bottom: 68.13%;
}
.Pixso-vector-1_1821 {
    width: 22.03%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1821.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 26.2%;
    right: 51.77%;
    top: 33.25%;
    bottom: 53.01%;
}
.Pixso-vector-1_1824 {
    width: 22.03%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1824.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 26.2%;
    right: 51.77%;
    top: 48.26%;
    bottom: 38%;
}
.Pixso-vector-1_1827 {
    width: 22.03%;
    height: 13.74%;
    background-image: url(@/assets/images/Vector_1_1827.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 26.2%;
    right: 51.77%;
    top: 63.37%;
    bottom: 22.89%;
}
.Pixso-vector-1_1830 {
    width: 2.13%;
    height: 2.23%;
    background-image: url(@/assets/images/Boolean_operation_1_1830.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 7.92%;
    right: 89.95%;
    top: 20.45%;
    bottom: 77.32%;
}
.Pixso-group-6_18 {
    width: 35.89%;
    height: 2.22%;
    position: absolute;
    left: 58.59%;
    right: 5.52%;
    top: 79.33%;
    bottom: 18.45%;
}
.Pixso-vector-6_19 {
    width: 87.37%;
    height: 100%;
    background-image: url(@/assets/images/Vector_6_19.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 12.63%;
    right: 0%;
    top: 0%;
    bottom: 0%;
}
.Pixso-paragraph-6_93 {
    font-size: 14px;
    font-family: "Helvetica Neue-Light";
    font-weight: 300;
    text-align: center;
    line-height: 22px;
    color: rgba(29, 33, 41, 1);
    width: 9.87%;
    height: 52.38%;
    position: absolute;
    left: 0%;
    right: 90.13%;
    top: 23.81%;
    bottom: 23.81%;
}
.Pixso-group-33_192 {
    width: 338px;
    height: 42px;
    position: absolute;
    left: 1476px;
    top: 90px;
}
.Pixso-vector-33_193 {
    width: 100%;
    height: 42px;
    background-image: url(@/assets/images/Vector_33_193.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 0%;
    right: 0%;
    top: 50%;
    transform: translateY(calc(-50% + 0px));
}
.Pixso-paragraph-33_194 {
    font-size: 16px;
    font-family: "Alibaba PuHuiTi-Regular";
    font-weight: 400;
    line-height: 20px;
    color: rgba(88, 83, 83, 1);
    width: 68.05%;
    height: 20px;
    position: absolute;
    left: 4.73%;
    right: 27.22%;
    top: 50%;
    transform: translateY(calc(-50% + 0px));
}
.Pixso-vector-33_195 {
    width: 7.39%;
    height: 25px;
    background-image: url(@/assets/images/Group_33_195.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 85.21%;
    right: 7.4%;
    top: 50%;
    transform: translateY(calc(-50% + 0.5px));
}
.Pixso-vector-33_515 {
    width: 10.62%;
    height: 3.7%;
    background-image: url(@/assets/images/Vector_33_515.png);
    background-size: 100% 100%;
    background-repeat: no-repeat;
    position: absolute;
    left: 34.95%;
    right: 54.43%;
    top: 10.25%;
    bottom: 86.05%;
}
.Pixso-paragraph-33_516 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 16.46%;
    right: 79.38%;
    top: 11.47%;
    bottom: 87.21%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-33_517 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 27.45%;
    right: 68.39%;
    top: 11.47%;
    bottom: 87.21%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-33_518 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 48.7%;
    right: 47.14%;
    top: 11.47%;
    bottom: 87.21%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-33_519 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 59.27%;
    right: 36.56%;
    top: 11.47%;
    bottom: 87.21%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-33_520 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 69.58%;
    right: 26.25%;
    top: 11.47%;
    bottom: 87.21%;
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-33_521 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 50%;
    top: 11.47%;
    bottom: 87.21%;
    transform: translateX(calc(-50% + 624px));
    white-space: pre;
    flex-grow: 0;
}
.Pixso-paragraph-33_522 {
    font-size: 20px;
    font-family: "FZDaHei-B02S-Regular";
    font-weight: 400;
    text-align: center;
    line-height: 25px;
    color: rgba(255, 255, 255, 1);
    width: auto;
    height: auto;
    position: absolute;
    left: 38.13%;
    right: 57.71%;
    top: 11.47%;
    bottom: 87.21%;
    white-space: pre;
    flex-grow: 0;
}
</style>
