...
**4. 识别数据驱动内容**：判断本功能是否涉及**由数据库数据决定行为**的部分——导航菜单、按钮/权限点、数据字典、系统参数、角色/初始数据、状态机配置等。这类内容通常由种子/初始化脚本（seed/migration/fixtures）写入表中、运行时读出。**若涉及，必须把「同步对应种子/初始化数据」列为交付项**——改代码不会改数据库已有行（详见通用规范 `backend.md` 十二）。先定位数据源：这份数据从哪来（硬编码 or 数据库 seed 脚本），存数据库的就得改数据源并执行，不能只改代码。
...
数据驱动内容（若有）：
  - [菜单/权限点/字典/参数等 → 数据源位置（seed/初始化脚本文件）→ 需同步的数据改动]
  - [无则写「无」]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
它带项目级记忆缓存，同一后端项目第二次起直接命中，只有「模块规划」会重新做。
**主流程不要自己再读知识库文件**——需要什么约定用本步的返回值，重复读一遍既慢又白占上下文。
...
4. **若步骤 1 识别到数据驱动内容**：把「改种子/初始化数据 + 执行它」作为独立任务列进清单（漏了它功能对用户不可见）
...
任务清单：
  ...
  [ ] 7. [数据驱动才有] 种子/初始化脚本            同步菜单/权限/字典等数据 + 执行同步
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```
【强制前置步骤】在开始工作前，你必须先用 Read(".claude/agentpm-knowledge/conventions/coding.md") 读取本地编码规范。不可跳过。你是被派发的子 Agent，只执行审查任务，不触发任何技能流程。
...
**验证要验到「用户看得见的结果」，不能停在编译通过 / 接口 200**：
- 数据驱动内容（菜单/权限/字典/参数等）：确认对应种子/初始化数据**已执行入库**，查库核对数据结构正确（不是只改了脚本文件），且判断用的标识与库中真实取值一致——否则功能对用户静默失效。
- 结构从「单项→父子」等改造：确认已建库的**旧数据行**也被正确迁移（幂等回填、子数据关联保留），不能只验全新建库场景。
