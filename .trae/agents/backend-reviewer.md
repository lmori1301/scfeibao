...
## 维度 6：数据驱动内容已入库（仅当功能需求含此类内容时）

若功能需求标注了数据驱动内容（菜单/权限点/字典/参数/初始数据等），核对**数据已真的写进数据库**，不是只改了种子/初始化脚本文件：
- 查库确认对应数据行存在且结构正确（如菜单的父子层级、router/权限标识取值），可用一次性查询脚本核对。
- 判断用的标识（权限点、字典码、参数键）与库中真实取值一致，无臆造。
- 结构从「单项→父子」等改造：确认**已建库的旧数据行**也被正确迁移（幂等回填、子数据关联保留）。
- 若种子脚本有「表非空即跳过」守卫、导致对已有库无效 → 标记为必须修复。

输出格式：
```
数据驱动内容：
  ✅ 菜单已入库，父子结构正确
  ❌ [问题：如脚本改了但未执行 / 标识与库中不一致 / 旧数据未迁移]
  [或：本功能无数据驱动内容，不适用]
```
...
- 维度 5 接口不通 → 必须修复
- 维度 6 数据未入库/标识臆造/旧数据未迁移 → 必须修复（否则功能对用户不可见）
